...
### [ Task ]
  1. `button` and `input` are inline elements (start in the same line).
  2. Add the `type` attribute with the value `submit` to the button to make it clear that it is a submit button.
