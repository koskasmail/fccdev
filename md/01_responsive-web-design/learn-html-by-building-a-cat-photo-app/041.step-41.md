...
### [ Task ]
  1. `required` attribute inside `input` element prevent a user from submitting your form without entering a value inside.
  2.  Instead, just add the word `required` to the `input` element, making sure there is space between it and other attributes.
